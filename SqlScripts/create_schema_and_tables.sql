...
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`text_collection_meta` (
  ...
  `purpose` ENUM('test', 'train') NOT NULL,
  `testTextCollectionMetaId` INT UNSIGNED NULL,
  PRIMARY KEY (`id`),
  INDEX `textCollectionMeta_into_itselfTestCollection_idx` (`testTextCollectionMetaId` ASC),
  CONSTRAINT `textCollectionMeta_into_itselfTestTextCollectionMeta`
    FOREIGN KEY (`testTextCollectionMetaId`)
...
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`raw_text_excerpt_location` (
  ...
  `lastCharacterPosition` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`detection` (
  ...
  `rawTextPairId` INT UNSIGNED NOT NULL,
  `name` TEXT NULL,
  `obfuscation` ENUM('none', 'random', 'translationChain', 'summary') NULL,
  `type` ENUM('artificial', 'translationChain') NULL,
...
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`seed_attributes` (
  ...
  `plagiarismClass` ENUM('none','direct','obfuscatedRandom','obfuscatedTranslation','obfuscatedSummary') NULL DEFAULT NULL,
  `percentualInDetection` DOUBLE NULL DEFAULT NULL,
  `cosine` DOUBLE NULL,
  `dice` DOUBLE NULL,
  `isMaxCosine` INT NULL,
  `maxCosineDiff` DOUBLE NULL,
  `meanMaxCosineDiff` DOUBLE NULL,
  `maxCosineNeighbour` DOUBLE NULL,
  `verticalCosineMaxDistance` DOUBLE NULL,
  `verticalCosineMaxMeasure` DOUBLE NULL,
  `isMaxDice` INT NULL,
  `maxDiceDiff` DOUBLE NULL,
  `meanMaxDiceDiff` DOUBLE NULL,
  `maxDiceNeighbour` DOUBLE NULL,
  `verticalDiceMaxDistance` DOUBLE NULL,
  `verticalDiceMaxMeasure` DOUBLE NULL,
  `lengthRatio` DOUBLE NULL,
  PRIMARY KEY (`seedId`),
...
-- -----------------------------------------------------
-- Table `plagiarism_detection`.`seeding_data_frame`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`seeding_data_frame` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `seedingDataId` INT UNSIGNED NULL,
  `descriptionDictionary` JSON NULL,
  `pickleDataFrame` LONGBLOB NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `seedDataFrame_into_seedingData_idx` (`seedingDataId` ASC),
  CONSTRAINT `seedDataFrame_into_seedingData`
    FOREIGN KEY (`seedingDataId`)
    REFERENCES `plagiarism_detection`.`seeding_data` (`id`)
    ON DELETE SET NULL
    ON UPDATE SET NULL)
...
-- -----------------------------------------------------
-- Table `plagiarism_detection`.`classifier_meta`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`classifier_meta` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `pickleClassifier` LONGBLOB NOT NULL,
  `definitionDictionary` JSON NOT NULL,
  `seedingDataFrameId` INT UNSIGNED NULL,
  `pickleExpectedPredictedList` LONGBLOB NULL,
  PRIMARY KEY (`id`),
  INDEX `classifier_into_seedingDataFrame_idx` (`seedingDataFrameId` ASC),
  CONSTRAINT `classifierMeta_into_seedingDataFrame`
    FOREIGN KEY (`seedingDataFrameId`)
    REFERENCES `plagiarism_detection`.`seeding_data_frame` (`id`)
    ON DELETE SET NULL
    ON UPDATE SET NULL)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `plagiarism_detection`.`evaluation` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `classifierId` INT UNSIGNED NULL,
  `creationDate` DATETIME NOT NULL,
  `report` MEDIUMTEXT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `evaluation_into_classifierMeta_idx` (`classifierId` ASC),
  CONSTRAINT `evaluation_into_classifierMeta`
    FOREIGN KEY (`classifierId`)
    REFERENCES `plagiarism_detection`.`classifier_meta` (`id`)
    ON DELETE SET NULL
    ON UPDATE SET NULL)
ENGINE = InnoDB;
